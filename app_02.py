...
def dynamic_timeline_forecasting(model, scaler, df: pd.DataFrame, start_date: pd.Timestamp, n_days: int) -> tuple:
    ...
    Fixes the weekend/holiday lookup crashes by using forward-fill matching (.asof)
    for requested points falling within historical boundaries.
    """
    db_max_date = df.index.max()
    target_start = pd.Timestamp(start_date)
    
    # Generate requested timeline frame
    ...
    # ── CASE 1: Timeline starts inside or intersects historical limits ──
    if target_start <= db_max_date:
        for idx, curr_date in enumerate(biz_dates):
            if curr_date <= db_max_date:
                # FIXED: Instead of strict .loc[curr_date] which crashes on weekends/holidays,
                # we slice up to the date and grab the last entry (.asof behavior)
                hist_match = df.loc[:curr_date]
                if not hist_match.empty:
                    actual_val = float(hist_match["Adj Close"].iloc[-1])
                else:
                    actual_val = float(df["Adj Close"].iloc[0])
                    
                preds_prices.append(actual_val)
                lower_bounds.append(actual_val)
                upper_bounds.append(actual_val)
            else:
                # If target window pushes past the database boundary
                ...
                # Incorporate intermediate predictions made in previous loop iterations
    ...
    # ── CASE 2: Timeline starts entirely in the future ──
    else:
        ...
        for g_date in gap_range:
            ...
            band_frac = np.clip(daily_vol * np.sqrt(bridge_step), 0, 0.5)
...
# Background processing logic loop
...
if model is not None and scaler_ok:
    try:
        ...
        # FIXED: Expose the actual tracking error to the UI if it happens to crash
        st.error(f"Simulation Engine Error Details: {_err}")
