...
with st.sidebar:
    ...
    gdrive_url = st.text_input(
        ...
        help="Paste the shareable link to your CNN-GRU .keras model. "
    ...
    if "model_loaded" in st.session_state and st.session_state["model_loaded"]:
        ...
        model_status_slot.info("ℹ️ No model loaded yet")
    ...
    entry_price  = st.number_input(
        "Your Entry Price ($)", min_value=0.0, value=0.0, step=0.01,
        help="Leave 0 to use today's close price automatically.",
    )
    position_qty = st.number_input("Position Size (shares)", min_value=1, value=10, step=1)
    risk_pct     = st.slider("Risk Tolerance (%)", 1, 20, 5,
                             help="Maximum % loss from entry you'll accept (sets stop-loss).")

    st.markdown("---")
    st.caption(f"Data: TSLA.csv + Yahoo Finance  |  Last row: {df.index[-1].strftime('%d %b %Y')}")
