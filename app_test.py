...
with tab3:
    ...
    # ── Flying Posters: render every poster as a single self-contained
    #    components.html block so the title button and Plotly chart are
    #    guaranteed to be inside the same DOM node. This is the only way
    #    to keep st.plotly_chart from escaping its parent div.
    import streamlit.components.v1 as components
    import json as _json

    def _poster_html(title_emoji: str, title_text: str, fig, float_class: str = "rb-poster-float-1") -> str:
        """
        Build a complete self-contained flying-poster HTML block.
        The Plotly figure is serialised to JSON and rendered via Plotly.js CDN
        inside the card, so title + chart are truly co-located in one div.
        """
...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .poster-card {{
    ...
    transform-style: preserve-3d;
    animation: {float_class} 6.5s ease-in-out infinite;
    overflow: visible;
  }}
  ...
  .poster-card::before {{
    ...
    inset: -1.5px;
    border-radius: 15px;
    background: linear-gradient(135deg, #3b82f6, #a855f7, #ffcc00, #3b82f6);
    -webkit-mask: linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0);
    -webkit-mask-composite: xor;
    mask-composite: exclude;
    background-size: 200% auto;
    animation: rb-border-rotate 5s linear infinite;
    opacity: 0.35;
    pointer-events: none;
  }}
  .poster-card:hover {{
    transform: translateY(-14px) rotateY(4deg) scale(1.02) !important;
    box-shadow: 0 24px 44px rgba(59,130,246,0.22) !important;
    animation-play-state: paused !important;
  }}
  .poster-card:hover::before {{ opacity: 1.0; inset: -2.5px; }}

  /* Title button — shimmer style (ReactBits: ShimmerButton) */
  .poster-title {{
    display: block;
    width: 100%;
    text-align: center;
    background: linear-gradient(90deg, #090c15, #17223b, #090c15);
    background-size: 200% auto;
    border: 1.2px solid rgba(255,204,0,0.45);
...
<body>
  <div class="poster-card">
    <div class="poster-title">{title_emoji} {title_text}</div>
    <div id="chart"></div>
  </div>
  <script>
    var figData = {fig_json};
    // Override bg to transparent so it matches the poster card
    figData.layout.paper_bgcolor = "rgba(0,0,0,0)";
    figData.layout.plot_bgcolor  = "rgba(12,16,28,0.45)";
    figData.layout.margin        = {{l:36, r:16, t:12, b:40}};
    figData.layout.height        = 430;
    figData.layout.autosize      = true;
    figData.layout.font          = {{color: "#e2e8f5", family: "JetBrains Mono, monospace", size: 11}};
    ...
    # Define posters — (emoji, title, figure, float_animation_class)
    posters = [
        ("🛰️", "CONTINUOUS CLOSE PRICING SEQUENCE",    fig1,  "rb-poster-float-1"),
        ("📊", "SEGMENTED BAR VOLUME DISTRIBUTION",    fig2,  "rb-poster-float-2"),
        ("🕯️", "HOLOGRAPHIC CANDLESTICK ENVELOPE",     fig3,  "rb-poster-float-3"),
        ("📈", "INTRADAY DISPERSION BOUNDS VARIANCE",  fig4,  "rb-poster-float-1"),
        ("📅", "MACRO ANNUALIZED PRICE ASSETS",        fig7,  "rb-poster-float-2"),
        ("❄️", "SEASONALITY DISTRIBUTION MATRICES",   fig8,  "rb-poster-float-3"),
    ]
    if fig5 is not None:
        posters.insert(4, ("📉", "MOMENTUM CONVERGENCE CONTOUR",     fig5, "rb-poster-float-2"))
    if fig6 is not None:
        posters.insert(5, ("📶", "STRENGTH VELOCITY RSI OSCILLATOR", fig6, "rb-poster-float-3"))
    if fig11 is not None:
        posters.append(   ("🧬", "ATTRIBUTE CORRELATION MATRIX",     fig11,"rb-poster-float-1"))

    # Render each poster in its own column group of 3 per row,
    # wrapped inside the flying-posters-viewport horizontal scroll strip.
    # We batch them: each row of 3 gets one horizontal flex container.
    POSTERS_PER_ROW = 3
    for row_start in range(0, len(posters), POSTERS_PER_ROW):
        row_posters = posters[row_start : row_start + POSTERS_PER_ROW]
        cols = st.columns(len(row_posters), gap="medium")
        for col, (emoji, title, fig_obj, float_cls) in zip(cols, row_posters):
            with col:
                html_block = _poster_html(emoji, title, fig_obj, float_cls)
                components.html(html_block, height=530, scrolling=False)
