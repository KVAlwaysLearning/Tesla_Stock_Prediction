  ...
  def clean_html(html_code: str) -> str:
      ...
      markdown parser from falsely wrapping multiline indented HTML segments, 
      while preserving newlines to ensure script syntax and comments remain functional."""
      if not html_code:
          return ""
      # Strip HTML comments to avoid parser confusion
      html_code = re.sub(r'<!--.*?-->', '', html_code, flags=re.DOTALL)
      # Process line-by-line to strip outer whitespace and skip blank lines, keeping newlines
      lines = []
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  with tab3:
      ...
      # Compile the cards using self-contained nested HTML structures
      def render_poster_html(title, fig):
          fig.update_layout(
              paper_bgcolor="rgba(0,0,0,0)",
              plot_bgcolor="rgba(0,0,0,0)",
          )
          # Disable default ModeBar for clean presentation, enable responsive width
          fig_html = fig.to_html(include_plotlyjs=False, full_html=False, config={'responsive': True, 'displayModeBar': False})
          return f"""
          <div class="flying-poster-card" style="display: flex; flex-direction: column; align-items: center; justify-content: flex-start; min-height: 550px; overflow: hidden; position: relative;">
              <div class="rb-graph-title-btn">{title}</div>
      ...
      # Poster 1
      posters_html_list.append(render_poster_html("🛰️ CONTINUOUS CLOSE PRICING SEQUENCE", fig1))
      
      # Poster 2
      posters_html_list.append(render_poster_html("📊 SEGMENTED BAR VOLUME DISTRIBUTION", fig2))
      
      # Poster 3
      posters_html_list.append(render_poster_html("🕯️ HOLOGRAPHIC CANDLESTICK ENVELOPE", fig3))
      
      # Poster 4
      posters_html_list.append(render_poster_html("📈 INTRADAY DISPERSION BOUNDS VARIANCE", fig4))
      
      # Poster 5
      if fig5 is not None:
          posters_html_list.append(render_poster_html("📉 MOMENTUM CONVERGENCE CONTOUR", fig5))
          
      # Poster 6
      if fig6 is not None:
          posters_html_list.append(render_poster_html("📶 STRENGTH VELOCITY RSI OSCILLATOR", fig6))
          
      # Poster 7
      posters_html_list.append(render_poster_html("📅 MACRO ANNUALIZED PRICE ASSETS", fig7))
      
      # Poster 8
      posters_html_list.append(render_poster_html("❄️ SEASONALITY DISTRIBUTION MATRICES", fig8))
      
      # Poster 9
      if fig11 is not None:
          posters_html_list.append(render_poster_html("🧬 ATTRIBUTE CORRELATION MATRIX", fig11))

      # Join them inside the scrollable viewport and inject Plotly CDN once
      combined_body = "\n".join(posters_html_list)
      master_viewport_html = f"""
      <script src="https://cdn.plot.ly/plotly-2.24.4.min.js"></script>
      <div class="flying-posters-viewport">
          {combined_body}
      </div>
      """
      st.markdown(master_viewport_html, unsafe_allow_html=True)
